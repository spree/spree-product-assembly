      ...
      if variant.should_track_inventory?
        ...
        variant.product.parts.each do |part|
          if part.should_track_inventory?
            lowest_value = part.stock_items(:count_on_hand)/variant.product.count_of(part) < lowest_value ? part.stock_items(:count_on_hand)/variant.product.count_of(part) : lowest_value
          end
        end
        return lowest_value
      else
