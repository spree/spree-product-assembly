module Spree
  module Stock
    describe Coordinator, type: :model do
      ...
      context "bundle variant is added to existing order, which already contains the bundle" do
        include_context "product is ordered as individual and within a bundle"

        before { StockItem.update_all 'count_on_hand = 10' }

        context "bundle quantity is bigger than 1" do
      ...
      context "order shares variant as individual and within bundle" do
        ...
        let(:part_variant) { bundle.master.parts.first }
        before { StockItem.update_all 'count_on_hand = 10' }

        context "bundle part requires more units than individual product" do

          before { order.contents.add(part_variant, 5) }
